...
namespace LeitorDeNotas.ClearArch.Api.Services;
...
public sealed class NotaFiscalXmlParser : INotaFiscalXmlParser
{
    public NotaFiscal Parse(string xmlContent)
    {
        ...
        var chaveAcesso = infNFe.Attribute("Id")?.Value ?? string.Empty;
        ...
        if (!itens.Any())
        {
            itens.Add(NotaFiscalItem.Criar(Guid.Empty, "Item padrão", "Produto", 1, 0));
        }

        return NotaFiscal.Criar(chaveAcesso, serie, dataEmissao, tipo, itens);
    }
}
